...
<div action="smy"STYLE="margin-left: 1050px;border: #2aabd2 solid 1px;width: auto;margin-top: 20px;background-color:lightblue;border-radius: 15px;">
    <a  href="toOrder2?id=${requestScope.id}&flag=1" class="btn btn-link">待发货订单</a>
    <a  href="toOrder2?id=${requestScope.id}&flag=2" class="btn btn-link">已发货订单</a>
    <a href="toOrder2?id=${requestScope.id}&flag=3">已完成订单</a>

    <a href="toOrder2?id=${requestScope.id}&flag=5" >管理商品</a>
    <a href="inf" >退出登录</a>
</div>
...
<div style="border: #4cae4c 1px solid; width: 900px;height: auto;margin-left: 300px;background-color: white; border-radius:80px;margin-top: 50px;" id="ar">
    <table border="1px" style="text-align: center;margin:50px;margin-left:200px;position: center;">
        ...
        <th colspan="" STYLE="width: 80px;text-align: center;">下单时间</th>
        <th STYLE="text-align: center;">图片</th>
        <th STYLE="text-align: center">书名</th>
        <th colspan="2"  STYLE="text-align: center;width: 60px;">价钱</th>
        <th colspan="" STYLE="width: 80px;text-align: center;"> 完成时间</th>
        <th  colspan="2" STYLE="text-align: center;">操作</th>
        <c:forEach var="item" items="${requestScope.get('list')}">
            <div class="row-fluid" style="float: left;" >
                <tr>
                    <td STYLE="color:darkslategrey;width: 30px;">${item.getCreattime()}</td>
                    ...
                    <td STYLE="color:darkslategrey;width: 30px;">${item.getEndtime()}</td>
                    <td colspan="4" style="width: 80px;">
                        <form action="tosend" method="post"><br>
                            <input type="text" value="${item.getOrderid()}" name="orderid" hidden="true"/>
                            <button type="submits" class="btn btn-danger">发货</button>
                        </form>
                    </td>
                </tr>
            </div>
